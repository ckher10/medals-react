...
function App() {
  ...
  const totalMedals = countries.reduce(
    (acc, country) => acc + country.gold + country.silver + country.bronze,
    0,
  );

  function addCountry(name) {
    const highestId =
      countries.reduce((maxId, currentObject) => {
        return Math.max(maxId, currentObject.id);
      }, 0) + 1;
  ...
  function increaseMedal(countryId, medalCategory) {
    ...
    const selectedCountry = newCountries.find(
